...
Помимо прозы, на вкладках (напр. `?tab=heritages`) также присутствуют `<pf2-table>` блоки
с дополнительными связанными сущностями. Вывод: детальные страницы пригодны для извлечения
описательного текста и, при необходимости, парсинга прозы регулярками, но **структурированные
механические значения надёжнее брать из JSON `items` индекс-страницы.**
...
`feat` (навыковый фит с прямой ссылкой, Acolyte: Student of the Canon → `feat/847`),
